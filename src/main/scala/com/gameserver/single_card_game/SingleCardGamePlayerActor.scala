...
import com.gameserver.deck.Card
...
class SingleCardGamePlayerActor(player: Player, dealerActorRef: ActorRef)(implicit ec: ExecutionContext) extends Actor {

  val logger = LoggerFactory.getLogger(this.getClass)

  private var originalSender: Option[ActorRef] = None

  var cardDrawn: Option[Card] = None
  ...
  def playing: Receive = {
    case card: Card =>
      ...
      cardDrawn = Some(card)
      player.sendMessage(s"Card drawn is ${card.toString}")
    ...
    case inputMessage: TextMessage => // Incoming message from player
      ...
      if(PlayerDecision.allS.contains(playerDesisionString)) {
        cardDrawn.foreach(
          card =>
            dealerActorRef ! SingleCardGamePlayerDecision(player, card, PlayerDecision.withName(playerDesisionString))
        )
      } else {
        val message =
    ...
    case gameResult: GameResult =>
      ...
      // Sennding game result to player
